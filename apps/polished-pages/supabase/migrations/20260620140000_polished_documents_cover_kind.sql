-- Allow saving AI-designed book covers to the document library.
...
alter table polished.documents add constraint documents_kind_check
  check (kind in ('cv','cover-letter','book','tailored','cover'));
...
begin
  ...
  if p_kind not in ('cv','cover-letter','book','tailored','cover') then raise exception 'invalid kind'; end if;
