...
| 6 | Search excellence | ◐ Cross-field, multi-word, relevance-ranked search (earlier) + institution & education navigation; deeper per-listing language/collection/institution **filters** are the documented next step |
| 7 | Trending knowledge | ✅ Trending institutions by real engagement (views + downloads); most-translated and recently-published shelves |
| 8 | Marketplace trust | ✅ Verified badges carried through institution discovery; institutional provenance via branded storefronts and stats |
...
| **Publisher** | 7.5 | Branded storefronts with catalog/series/language stats, featured + trending institution placement, multilingual surfacing. Gap: per-listing publisher provenance on catalog cards. |
| **School** | 7.0 | Educational discovery band, curriculum/teacher-resource surfaces, institution showcases. Gap: dedicated primary/secondary structuring depends on richer category data. |
...
| **Marketplace maturity** | 7.0 | The ecosystem layer (institutions, multilingual, trending, trust) is in place over a solid creator marketplace. The binding constraint is real content, not structure. |
...
1. **Per-listing provenance & filters (Circle 6/8).** Extend `polished_catalog`
   to carry the owning institution and `edition_language`, enabling
   "Published by {institution}" on every card and true language/institution/
   collection filtering of individual listings.
2. **Trending collections & educational content (Circle 7).** Surface trending
   at the collection and category level once engagement data exists.
3. **A live relationship graph (Circle 9).** Upgrade the ecosystem band into an
