...
// record verification.
const DEST_ROUTES = [LEARN_ROUTE, LIBRARY_ROUTE, INDUSTRIES_ROUTE, VERIFY_ROUTE];

const NotFound: React.FC = () => {
  const nav = useNavigate();
  const { pathname } = useLocation();
  useReveal();
  const c = useMarketing7Copy().notFound;
  React.useEffect(() => {
      ...
      <main>
        <section className="relative overflow-hidden border-t border-white/[0.06] px-6 py-28 lg:px-12">
          ...
          <div className="relative mx-auto max-w-[1000px]">
            ...
            <h1 className="mt-5 max-w-2xl font-serif text-[2.4rem] font-bold leading-[1.08] tracking-tight text-[#f4efe3] sm:text-[3.1rem]">{c.title}</h1>
            <p className="mt-6 max-w-2xl text-[16.5px] leading-relaxed text-white/65">{c.lead}</p>

            <div className="mt-10 grid gap-3 sm:grid-cols-2">
              {c.dests.map((d, i) => (
                <button key={DEST_ROUTES[i]} onClick={() => nav(DEST_ROUTES[i])}
            ...
            <div className="mt-10">
              <button onClick={() => nav("/")} className="group inline-flex items-center gap-2 text-[13.5px] font-semibold uppercase tracking-wide text-gold-300 transition hover:text-gold-200">
                {c.home} <Chevron className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" />
              </button>
            </div>
          </div>
        </section>
