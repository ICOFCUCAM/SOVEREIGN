      ...
      <Card className="mb-6 p-6">
        <div className="flex items-end justify-between">
          <div>
            ...
            <div className="mt-1 text-5xl font-bold tabular-nums text-white">{c ? `${rate}%` : "·"}</div>
            <div className="mt-1 text-[12px] text-white/45">{c ? `${c.compliant} of ${c.governed} governed publications carry a COMPLIANT certificate` : ""}</div>
          </div>
        ...
        <div className="mt-4 h-2 overflow-hidden rounded bg-white/10"><div className={`h-full transition-all ${rate >= 100 ? "bg-emerald-400" : rate >= 90 ? "bg-amber-400" : "bg-red-400"}`} style={{ width: `${rate}%` }} /></div>
      </Card>
