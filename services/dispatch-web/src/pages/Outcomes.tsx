...
const Outcomes: React.FC = () => {
  ...
  const c = useMarketing2Copy().outcomes;
  useReveal();
  return (
    <div className="relative min-h-full bg-[#070707] text-white">
      ...
      <main>
        ...
        <section className="scroll-mt-24 border-t border-white/[0.06] px-8 py-24 lg:px-12">
          <SectionHead index="01" kicker={c.baKicker} title={c.baTitle} sub={c.baSub} />
          <div className="mx-auto mt-14 grid max-w-5xl gap-6 sm:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-7">
              <div className="text-[12px] font-semibold uppercase tracking-[0.2em] text-white/40">{c.beforeLabel}</div>
              <ul className="mt-4 space-y-3 text-[14px] text-white/55">{c.before.map((x) => <li key={x} className="flex gap-3"><span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-white/25" />{x}</li>)}</ul>
            </div>
            <div className="rounded-2xl border border-gold-400/25 bg-gradient-to-b from-gold-400/[0.06] to-transparent p-7">
              <div className="text-[12px] font-semibold uppercase tracking-[0.2em] text-gold-400">{c.afterLabel}</div>
              <ul className="mt-4 space-y-3 text-[14px] text-white/80">{c.after.map((x) => <li key={x} className="flex gap-3"><Chevron className="mt-1 h-3.5 w-3.5 shrink-0 text-gold-400" />{x}</li>)}</ul>
            </div>
          </div>
        </section>
        ...
        <section className="scroll-mt-24 border-t border-white/[0.06] bg-gradient-to-b from-white/[0.025] to-white/[0.008] px-8 py-24 lg:px-12">
          <SectionHead index="02" kicker={c.scKicker} title={c.scTitle} sub={c.scSub} />
        ...
        <section className="border-t border-white/[0.06] px-8 py-20 lg:px-12">
          <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4">
            <div className="text-xl font-semibold text-white/80">{c.closeLine}</div>
            <button onClick={() => nav(TRUST_ROUTE)} className="inline-flex items-center gap-2 rounded-full bg-gold-400 px-6 py-3 text-[13px] font-semibold uppercase tracking-wide text-black transition hover:bg-gold-300">{c.closeCta} <Chevron className="h-3.5 w-3.5" /></button>
          </div>
        </section>
      </main>
      <PublicFooter />
