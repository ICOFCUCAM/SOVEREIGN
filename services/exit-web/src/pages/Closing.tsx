...
const Closing: React.FC = () => {
  ...
  const pending    = CLOSING.filter((c) => c.status === "pending").length;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <Kpi label="Blockers"     value={String(blocked)}               sub="founder-action required" />
      </div>
      ...
      <div className="mt-10">
        ...
        <p className="mt-3 text-xs text-white/40">{pending} item{pending === 1 ? "" : "s"} pending — all founder-owned items must be cleared by the morning of closing.</p>
      </div>
    </div>
  );
};
