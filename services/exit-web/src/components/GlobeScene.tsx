...
const GlobeScene: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ---- 7-LAYER INTELLIGENCE REACTOR (the foundation under the globe) ----
      // The globe is GENERATED by this machine. Built back→front as seven
      // distinct layers so it reads as a living command reactor, not circles:
      ...
      for (let k = 0; k < procRings.length; k++) {
        ...
        const rad = R * pr.r;
        ctx.strokeStyle = `rgba(140,226,255,${pr.a})`;
        ctx.lineWidth = pr.lw;
        ctx.beginPath();
        ctx.ellipse(cx, py, rad, rad * FLAT, 0, 0, TAU);
        ctx.stroke();
        // rotating bright segment on each processing ring → "machine" motion
        const seg = rt * pr.sp;
        ctx.strokeStyle = `rgba(205,249,255,${(pr.a + 0.35).toFixed(3)})`;
        ctx.lineWidth = pr.lw + 0.6;
        ctx.beginPath();
        ctx.ellipse(cx, py, rad, rad * FLAT, 0, seg, seg + 0.7);
        ctx.stroke();
      }

      // ── LAYER 5 · Circuit lanes ─ tiny traces/data-lanes between rings ──────
      for (let k = 0; k < procRings.length - 1; k++) {
        const r0 = R * procRings[k].r;
        const r1 = R * procRings[k + 1].r;
        const lanes = 5 + k;
        for (let l = 0; l < lanes; l++) {
          const ang = (l / lanes) * TAU + k * 0.4 + rt * 0.0004;
          const cosA = Math.cos(ang), sinA = Math.sin(ang) * FLAT;
          // radial connector trace
          ctx.strokeStyle = "rgba(110,205,245,0.10)";
          ctx.lineWidth = 0.5;
          ctx.beginPath();
          ctx.moveTo(cx + cosA * r0, py + sinA * r0);
          ctx.lineTo(cx + cosA * r1, py + sinA * r1);
          ctx.stroke();
          // a data packet creeping outward along the lane
          const fp = ((rt * 0.004 + l * 0.21 + k * 0.13) % 1);
          const pr2 = r0 + (r1 - r0) * fp;
          ctx.fillStyle = `rgba(200,246,255,${(0.5 * (1 - fp)).toFixed(3)})`;
          ctx.fillRect(cx + cosA * pr2 - 0.5, py + sinA * pr2 - 0.5, 1, 1);
        }
      }

      // ── LAYER 6 · Scan sweeps ─ rotating radar bands ────────────────────────
      for (let s = 0; s < 2; s++) {
        const rad = R * (0.86 + s * 0.54);
        const ticks = 56 + s * 28;
        for (let m = 0; m < ticks; m++) {
          ...
          const bright = m % 8 === 0;
          ctx.fillStyle = `rgba(175,240,255,${bright ? 0.42 : 0.13})`;
          ctx.fillRect(mx - 0.5, my - 0.5, bright ? 1.5 : 0.8, bright ? 1.5 : 0.8);
        }
        const sweep = (reduce ? 0.2 + s : (rt * (s ? -0.011 : 0.013))) % TAU;
        // a fading wedge trailing the scan head
        for (let w = 0; w < 5; w++) {
          ctx.strokeStyle = `rgba(205,249,255,${(0.5 - w * 0.09).toFixed(3)})`;
          ctx.lineWidth = 1.3;
          ctx.beginPath();
          ctx.ellipse(cx, py, rad, rad * FLAT, 0, sweep - w * 0.12, sweep - w * 0.12 + 0.1);
          ctx.stroke();
        }
      }

      // ── LAYER 3 · Inner command ring ─ small precision ring + calibration ───
      const cmdRad = R * 0.26;
      ctx.strokeStyle = "rgba(210,250,255,0.7)";
      ctx.lineWidth = 1.3;
      ctx.beginPath(); ctx.ellipse(cx, py, cmdRad, cmdRad * FLAT, 0, 0, TAU); ctx.stroke();
      const cmdSpin = rt * 0.01;
      for (let m = 0; m < 36; m++) {
        const ang = (m / 36) * TAU + cmdSpin;
        const big = m % 9 === 0;
        const r0 = cmdRad * (big ? 0.82 : 0.9);
        const mx0 = cx + Math.cos(ang) * r0, my0 = py + Math.sin(ang) * r0 * FLAT;
        const mx1 = cx + Math.cos(ang) * cmdRad, my1 = py + Math.sin(ang) * cmdRad * FLAT;
        ctx.strokeStyle = `rgba(205,248,255,${big ? 0.7 : 0.32})`;
        ctx.lineWidth = big ? 1.1 : 0.6;
        ctx.beginPath(); ctx.moveTo(mx0, my0); ctx.lineTo(mx1, my1); ctx.stroke();
      }
      // two short rotating arc segments inside the command ring
      for (let s = 0; s < 2; s++) {
        const a0 = -cmdSpin * (s ? 1.6 : -1.2) + s * Math.PI;
        ctx.strokeStyle = "rgba(225,251,255,0.8)";
        ctx.lineWidth = 1.6;
        ctx.beginPath();
        ctx.ellipse(cx, py, cmdRad * 0.66, cmdRad * 0.66 * FLAT, 0, a0, a0 + 1.1);
        ctx.stroke();
      }

      // ── LAYER 2 · Projection beam ─ volumetric light column into the globe ──
      const beamTop = cy - R * 0.15;
      const beam = ctx.createLinearGradient(0, py, 0, beamTop);
      beam.addColorStop(0, "rgba(165,240,255,0.34)");
      beam.addColorStop(0.5, "rgba(145,230,255,0.13)");
      beam.addColorStop(1, "rgba(145,230,255,0)");
      ctx.fillStyle = beam;
      ctx.beginPath();
      ctx.moveTo(cx - R * 0.11, py);
      ctx.lineTo(cx + R * 0.11, py);
      ctx.lineTo(cx + R * 0.42, beamTop);
      ctx.lineTo(cx - R * 0.42, beamTop);
      ctx.closePath();
      ctx.fill();
      // a brighter inner core column + rising energy motes
      const beam2 = ctx.createLinearGradient(0, py, 0, beamTop);
      beam2.addColorStop(0, "rgba(225,251,255,0.40)");
      beam2.addColorStop(1, "rgba(200,246,255,0)");
      ctx.fillStyle = beam2;
      ctx.beginPath();
      ctx.moveTo(cx - R * 0.04, py);
      ctx.lineTo(cx + R * 0.04, py);
      ctx.lineTo(cx + R * 0.13, beamTop);
      ctx.lineTo(cx - R * 0.13, beamTop);
      ctx.closePath();
      ctx.fill();
      for (let mIdx = 0; mIdx < 10; mIdx++) {
        const fp = ((rt * 0.006 + mIdx * 0.1) % 1);
        const yy = py + (beamTop - py) * fp;
        const spread = R * (0.05 + 0.34 * fp);
        const xx = cx + Math.sin(mIdx * 2.3 + rt * 0.02) * spread;
        ctx.fillStyle = `rgba(225,251,255,${(0.55 * (1 - fp)).toFixed(3)})`;
