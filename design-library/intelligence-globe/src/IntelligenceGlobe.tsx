...
const IntelligenceGlobe: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ---- holographic projection reactor (platform → beam → globe) --------
      // The globe is GENERATED by the platform: a volumetric cyan beam rises
      // from the reactor core into the sphere, surrounded by many concentric
      // command rings, rotating scan rings and radial markings.
      ctx.globalCompositeOperation = "lighter";
      const py = cy + R * 0.98; // platform sits right under the globe
      const FLAT = 0.20; // ellipse foreshortening

      // (a) volumetric projection beam — a soft cone of light from the reactor
      // core up into the globe (one coherent beam, not the old harsh strands).
      const beamTop = cy - R * 0.2;
      const beamTopHalf = R * 0.46;
      const beamBotHalf = R * 0.12;
      const beam = ctx.createLinearGradient(0, py, 0, beamTop);
      beam.addColorStop(0, "rgba(150,236,255,0.30)");
      beam.addColorStop(0.5, "rgba(140,228,255,0.12)");
      beam.addColorStop(1, "rgba(140,228,255,0)");
      ctx.fillStyle = beam;
      ctx.beginPath();
      ctx.moveTo(cx - beamBotHalf, py);
      ctx.lineTo(cx + beamBotHalf, py);
      ctx.lineTo(cx + beamTopHalf, beamTop);
      ctx.lineTo(cx - beamTopHalf, beamTop);
      ctx.closePath();
      ctx.fill();

      // (b) many concentric command rings — the platform is almost as important
      // as the globe. A pair of bright rings, the rest calm; even spacing.
      const baseRings = [0.26, 0.44, 0.62, 0.82, 1.04, 1.28, 1.54, 1.82];
      for (let k = 0; k < baseRings.length; k++) {
        const rad = R * baseRings[k];
        const lit = k === 2 || k === 5;
        const a = lit ? 0.7 : 0.22 + 0.04 * Math.sin(k);
        ctx.strokeStyle = lit ? `rgba(200,248,255,${a})` : `rgba(135,222,255,${a.toFixed(3)})`;
        ctx.lineWidth = lit ? 1.5 : 0.7;
        ctx.beginPath();
        ctx.ellipse(cx, py, rad, rad * FLAT, 0, 0, TAU);
        ctx.stroke();
      }
      // (c) two counter-rotating scan rings with radial tick markings
      for (let s = 0; s < 2; s++) {
        const rad = R * (0.82 + s * 0.46);
        const spin = reduce ? 0.3 + s : t * (s ? -0.006 : 0.006);
        const ticks = 48 + s * 24;
        for (let m = 0; m < ticks; m++) {
          ...
          const bright = m % 8 === 0;
          ctx.fillStyle = `rgba(180,242,255,${bright ? 0.5 : 0.16})`;
          ctx.fillRect(mx - 0.5, my - 0.5, bright ? 1.6 : 0.9, bright ? 1.6 : 0.9);
        }
        // a bright arc sweeping the scan ring
        const sweep = (reduce ? 0.2 + s * 0.3 : (t * (s ? -0.01 : 0.012)) % TAU);
        ctx.strokeStyle = `rgba(210,250,255,0.6)`;
        ctx.lineWidth = 1.4;
        ctx.beginPath();
        ctx.ellipse(cx, py, rad, rad * FLAT, 0, sweep, sweep + 0.5);
        ctx.stroke();
      }
      // (d) reactor core: layered glow + tight hot centre (the beam's source)
      const focus = ctx.createRadialGradient(cx, py, 0, cx, py, R * 0.66);
      focus.addColorStop(0, "rgba(215,251,255,0.62)");
      focus.addColorStop(0.4, "rgba(140,228,255,0.18)");
      focus.addColorStop(1, "rgba(120,220,255,0)");
      ctx.fillStyle = focus;
      ctx.beginPath(); ctx.ellipse(cx, py, R * 0.66, R * 0.17, 0, 0, TAU); ctx.fill();
      const dot = ctx.createRadialGradient(cx, py, 0, cx, py, R * 0.13);
      dot.addColorStop(0, "rgba(240,254,255,0.95)");
      dot.addColorStop(1, "rgba(185,246,255,0)");
      ctx.fillStyle = dot;
      ctx.beginPath(); ctx.ellipse(cx, py, R * 0.13, R * 0.055, 0, 0, TAU); ctx.fill();
      ctx.globalCompositeOperation = "source-over";
