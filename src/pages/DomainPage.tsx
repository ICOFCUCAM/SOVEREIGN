...
import { Shield, Sparkles, Rocket, TrendingUp, Globe, MessageCircle, DollarSign, ArrowLeft, CheckCircle2, Activity, Brain, Layers, Zap, Award } from 'lucide-react';

const DomainPage: React.FC = () => {
  const { domain } = useParams<{ domain: string }>();
  const navigate = useNavigate();
  const [d, setD] = useState<Domain | null>(null);
  const [val, setVal] = useState<ValuationReport | null>(null);
  const [loading, setLoading] = useState(true);
  const [valLoading, setValLoading] = useState(true);
  const [modal, setModal] = useState<null | 'inquiry' | 'offer' | 'buy_now'>(null);

  useEffect(() => {
    if (!domain) return;
    (async () => {
      const decoded = decodeURIComponent(domain);
      const { data } = await supabase.from('domains').select('*').eq('domain_name', decoded).maybeSingle();
      if (!data) { setLoading(false); return; }
      const dom = data as Domain;
      setD(dom);
      setLoading(false);

      // bump view
      supabase.from('domains').update({ view_count: (dom.view_count || 0) + 1 }).eq('id', dom.id).then(() => {});
      supabase.from('analytics_events').insert({ domain_id: dom.id, event_type: 'view', path: `/d/${decoded}` }).then(() => {});

      // valuation
      try {
        const { data: vd } = await supabase.functions.invoke('ai-valuation', { body: { domain: decoded } });
        if (vd) {
          setVal(vd as ValuationReport);
          // persist
          supabase.from('valuation_reports').insert({
            domain_id: dom.id,
            overall_score: vd.overall_score,
            brand_strength: vd.brand_strength,
            memorability: vd.memorability,
            seo_potential: vd.seo_potential,
            ai_relevance: vd.ai_relevance,
            market_alignment: vd.market_alignment,
            startup_viability: vd.startup_viability,
            sovereign_potential: vd.sovereign_potential,
            estimated_value_low: vd.estimated_value_low,
            estimated_value_high: vd.estimated_value_high,
            narrative: vd.narrative,
            slogan: vd.slogan,
            industry_category: vd.industry_category,
            confidence_score: vd.confidence_score,
            startup_concepts: vd.startup_concepts,
            model_used: vd.model_used,
          }).then(() => {});
        }
      ...
      <div className="min-h-screen">
        ...
        <div className="pt-32 px-4 max-w-2xl mx-auto text-center">
          <div className="glass-strong rounded-2xl p-12">
            <div className="text-7xl font-bold text-gradient-cyan mb-4">404</div>
            <div className="text-white text-2xl font-bold mb-2">Domain not registered</div>
            <p className="text-white/50 mb-6">This domain isn't in the sovereign registry yet. It may be available for acquisition.</p>
          ...
          <div className="grid lg:grid-cols-[1fr_320px] gap-8 items-start">
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setModal('buy_now')}
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {val.startup_concepts.map((c, i) => (
                    <div key={i} className="glass rounded-xl p-5 hover:glass-strong transition group">
                      ...
                      <button onClick={() => setModal('buy_now')} className="mt-4 w-full px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-mono uppercase tracking-wider text-cyan-300 transition flex items-center justify-center gap-1.5">
